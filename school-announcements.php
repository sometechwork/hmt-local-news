...
/**
 * Create a School Principal role with capabilities for announcements.
 */
function stw_sa_add_roles() {
	...
	add_role( 'school_principal', 'School Principal', $caps );
}
...
function stw_sa_enforce_school_term_on_save( $post_id, $post, $update ) {
	...
	// Only enforce for principals (admins can post anywhere).
	if ( ! in_array( 'school_principal', (array) $user->roles, true ) ) {
		return;
	}
